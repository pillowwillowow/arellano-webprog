...
const filteredUsers = users.filter((user) => {
  ...
  const validate = () => {
    ...
    if (!nextErrors.email && users.some((user) => user.id !== modal.id && user.email.trim().toLowerCase() === form.email.trim().toLowerCase())) {
      nextErrors.email = 'Email already exists.';
    }
    if (!nextErrors.userName && users.some((user) => user.id !== modal.id && user.userName.trim().toLowerCase() === userName)) {
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Stack direction='row' spacing={1} sx={{ py: 0.5}}>
          <Button size="small" variant="outlined" onClick={() => openAddModal(params.row)}>
            Edit
          </Button>
          <Button 
            size="small"
  ...
  return (
    ...
    >
        <Typography variant="h4" sx={{ fontFamily: "'Lexend', sans-serif", fontWeight: 500 }}>
          Users
        </Typography>
        <Button 
        variant="contained" 
    ...
    <Paper sx={{ p: {xs: 1.5, sm: 2}, minWidth: 0, overflowX: 'hidden' }}>
      {users.length ? (
